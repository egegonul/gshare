...
const unsigned globalCounterBits = 4;
...
bool predict(Branch_Predictor *branch_predictor, Instruction *instr)
{
    ...
    #ifdef GSHARE
    //get prediction
    uint64_t index = (branch_address >> instShiftAmt) ^ branch_predictor->global_history;
    unsigned local_index = getIndex(index, branch_predictor->local_history_table_mask);
    
    bool prediction = getPrediction(&(branch_predictor->local_counters[local_index]));
    
    //update counter and GHR
    if (instr->taken)
    {
        incrementCounter(&(branch_predictor->local_counters[local_index]));
        branch_predictor->global_history = (branch_predictor->global_history << 1) | 1;
    }
    else
    {
        decrementCounter(&(branch_predictor->local_counters[local_index]));
